...
// Define the name of your BLE device as it will appear to other devices
#define DEVICE_NAME "ESP32C3_Broadcast"

// Define the service UUID for your custom service (you can generate a random one)
// This UUID helps categorize your advertised data.
#define SERVICE_UUID "4FAFCD20-1F00-47B2-B35E-2C1F58500000"

// Define the characteristic UUID for the data you want to broadcast
// This UUID identifies the specific data being sent within the service.
#define CHARACTERISTIC_UUID "BEB5483E-36E1-4688-B7F5-EA07361B26A8"

// Global pointer to the BLEAdvertising object
BLEAdvertising *pAdvertising;

void setup() {
  // Initialize serial communication for debugging purposes
  Serial.begin(115200);
  Serial.println("Starting BLE Broadcast on ESP32 C3 Super Mini...");

  // Initialize the BLE device
  // This sets up the necessary hardware and software components for BLE.
  BLEDevice::init(DEVICE_NAME);

  // Create a BLE server
  // Although we are only advertising, a server is implicitly created for advertising functionality.
  BLEServer *pServer = BLEDevice::createServer();

  // Create a BLE service
  // This service will contain the characteristic with our "Hello World" message.
  BLEService *pService = pServer->createService(SERVICE_UUID);

  // Create a BLE characteristic
  // This characteristic will hold the data we want to broadcast.
  // BLECharacteristic::PROPERTY_READ allows other devices to read this characteristic.
  BLECharacteristic *pCharacteristic = pService->createCharacteristic(
                                         CHARACTERISTIC_UUID,
                                         BLECharacteristic::PROPERTY_READ
                                       );

  // Set the value of the characteristic to "Hello World!"
  // This is the actual data that will be broadcasted.
  pCharacteristic->setValue("Hello World!");

  // Start the service
  // Makes the service available for advertising.
  pService->start();

  // Get a pointer to the BLE advertising object
  pAdvertising = pServer->getAdvertising();

  // Create a BLE Advertising Data object
  // This object allows us to customize what information is included in the advertisement.
  BLEAdvertisementData advertisementData;

  // Set the service UUIDs that will be advertised
  // This tells scanning devices that this device offers the specified service.
  advertisementData.addServiceUUID(SERVICE_UUID);

  // Set the manufacturer specific data (optional, but good for custom messages)
  // This allows you to include arbitrary data in the advertisement.
  // Here, we're adding "Hello World!" as manufacturer data.
  // The first two bytes (0x0102) are a dummy manufacturer ID.
  // The rest is the "Hello World!" string.
  std::string manufacturerData = "Hello World!";
  advertisementData.setManufacturerData("0102" + BLEUtils::nonHex2Hex(manufacturerData));

  // Set the advertising data
  pAdvertising->setAdvertisementData(advertisementData);

  // Set the scan response data (optional, sent when a scanner requests more info)
  // This data is sent in response to an active scan request.
  BLEAdvertisementData scanResponseData;
  scanResponseData.setCompleteServices(BLEUUID(SERVICE_UUID)); // Add the service UUID to scan response
  scanResponseData.setShortName(DEVICE_NAME); // Add a short name to scan response
  pAdvertising->setScanResponseData(scanResponseData);

  // Start advertising
  // This makes the ESP32 visible to other BLE devices.
  pAdvertising->start();
  Serial.println("BLE Advertising started. Broadcasting 'Hello World!'");
}

void loop() {
  // The loop function can be used for other tasks, but for simple advertising, it can be empty.
  // The advertising will continue in the background.
  delay(1000); // Small delay to prevent busy-waiting, though not strictly necessary for advertising.
}
